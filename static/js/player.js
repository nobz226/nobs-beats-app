document.addEventListener('DOMContentLoaded', () => {
    ...
    const shouldArtworkSpin = (artworkElement) => {
        const artworkSrc = artworkElement.src || artworkElement.querySelector('img')?.src || '';
...
const updateArtworkSpinning = (isPlaying) => {
    ...
    const allShowcaseArtworks = document.querySelectorAll('.track-card .track-artwork');
    ...
    const restoreState = () => {
        ...
        if (savedState) {
            ...
            if (player.audio.readyState > 1) {
                ...
                player.audio.addEventListener('canplay', restorePlayback, { once: true });
            }
        }
    };
